...
return new class extends Logic\Admin\BaseController
{
    ...
    public function run()
    {
        (new NotifyValidate())->paramsCheck('put', $this->request, $this->response);
        ...
        if($msg>0){
            ...
            if ($result) {
                ...
                $str = "客户名称：{$customer_name} 前端域名:{$www_notify} 后台域名：{$admin_notify}";
                (new Log($this->ci))->create(null, null, Log::MODULE_CUSTOMER, '客户域名设置', '新增域名', "添加", $sta, $str);
                /*============================================================*/
