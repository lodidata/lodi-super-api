...
class LDF extends BASES {
    ...
    public function sign(){
        ...
        $ms = 'version=1.0&'.'customerid='.$this->partnerID.'&total_fee='.number_format($this->money,2).'&sdorderno='.$this->orderID.'&notifyurl='.$this->notifyUrl.'&returnurl='.$this->returnUrl.'&'.$this->key;
        return md5($ms);
    ...
    public function parseRE(){
        ...
        if($this->money < 500 && $this->data->bank_data == 'alipaywap'){  //支付宝H5充值金额必须 大于或等于500
            $this->return['code'] = 5;
            $this->return['msg'] = 'LDF:订单金额小于规定的最低额度==>500' ;
    ...
    public function returnVerify($parameters = array()) {
        ...
        $str = 'customerid='.$parameters['customerid'].'&status='.$parameters['status'].'&sdpayno='.$parameters['sdpayno'].'&sdorderno='.$parameters['sdorderno'].'&total_fee='.$parameters['total_fee'].'&paytype='.$parameters['paytype'].'&'.$key;
