...
return new class extends Action
{
    ...
    public function run()
    {
        ...
        GameApi::addElkLog(['VerifySession' => $params], 'IG2');
        if (!isset($params['operator_token']) || !isset($params['secret_key']) || !isset($params['custom_parameter']) || empty($params['custom_parameter']) || !@hex2bin($params['custom_parameter'])) {
            ...
            if (!$tid) {
                ...
                $gameClass = new Logic\Game\Third\IG2($this->ci);
                $config = $gameClass->initConfigMsg('IG2');
                if ($params['operator_token'] != $config['key'] || $params['secret_key'] != $config['pub_key']) {
                    ...
                    if (empty($www_notify)) {
                        ...
                        //推送消息
                        $url = rtrim($www_notify, '/') . '/game/third/ig2/VerifySession';
